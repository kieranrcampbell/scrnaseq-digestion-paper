...
```{r}
ids <- filter(design, temp_de_comparison == params$comparison) %>% 
  .$id
```
...
```{r}
sce_paths <- sapply(ids, function(id) glue("../../data/scesets/{params$cellranger_version}/{id}_sceset_{params$cellranger_version}_qc.rds"))
```
...
```{r}
rowdata_to_remove <- names(rowData(sces[[1]]))[-c(1:5)]
...
sces <- lapply(sces, function(sce) {
  ...
  reducedDims(sce)[['UMAP']] <- NULL # not computed for all samples
  sce
})
```
...
plotPCA(sce, colour_by = "total_features_by_counts", ncomponents = 3)
...
```{r}
plotColData(sce, x = "digestion_temperature", y = "total_features_by_counts")
```
...
df_limma <- dplyr::mutate(df_limma,
...
df_text <- top_n(df_limma, 25, desc(adj.P.Val))
