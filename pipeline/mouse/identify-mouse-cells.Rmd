...
```{r}
sce_dir <- here("data", "human-mouse-sces", "filtered_temperature_rdata_v3" )

sces_filenames <- dir(sce_dir)
...
```{r}
sces_human <- lapply(human_names, function(x) readRDS(file.path(sce_dir, x)))
sces_mouse <- lapply(mouse_names, function(x) readRDS(file.path(sce_dir, x)))
...
Let's add some crappy annotation
...
# forcats::fct_reorder(sample_id, human_mouse_diff)
filter(dfs, sum > 0) %>% 
  ggplot(aes(x = sample_id, y = diff_to_sum)) + 
  # geom_boxplot() +
  geom_boxplot() +
  # scale_y_log10() +
  coord_flip() +
  labs(y = "(human counts - mouse counts) / (human counts + mouse counts)", x = "Sample") +
  facet_grid(sample_type ~ ., scales = "free_y", space = "free_y") +
  theme_bw(base_size = 9) +
  theme(axis.title.x = element_text(size = 8))

ggsave(here("figs", "mouse", "human_mouse_boxplot.png"), width = 5, height = 8)
```
...
ggsave(here("figs", "mouse", "human_mouse_histogram_pdxonly.png"), width = 8, height = 5)
```
...
ggsave(here("figs", "mouse", "pct_cells_mouse.png"), width = 5, height = 8)
```

```{r}
write_csv(dfs, here("data", "deliverables", "human_mouse_proportions.csv"))
```
