...
gen_stats <- function(tt) {
  ...
  stats$pct_signif <- round2(100 * stats$total_signif / stats$total_genes)
  ...
  stats$pct_core_signif <- round2(100 * stats$core_signif / length(coregenes))
  stats$n_core_up <- dplyr::filter(tt_core_signif, logFC > 0) %>% nrow()
  stats$n_core_down <- dplyr::filter(tt_core_signif, logFC < 0) %>% nrow()
  
  names(stats) <- paste0(names(stats), "_", tt$comparison[1])
  tibble(
...
df_stats <- bind_rows(
  ...
  tibble(description = "de_correlation_time_method", statistic = as.character(correlation_time_method))
)
